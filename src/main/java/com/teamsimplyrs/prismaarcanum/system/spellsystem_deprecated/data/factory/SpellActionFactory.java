package com.teamsimplyrs.prismaarcanum.system.spellsystem_deprecated.data.factory;

import com.google.gson.JsonArray;
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.teamsimplyrs.prismaarcanum.system.spellsystem_deprecated.actions.*;
import com.teamsimplyrs.prismaarcanum.system.utils.SpellUtils;
...
public class SpellActionFactory {
    ...
    private static GenericAction createSpellAction(JsonObject json, String type) {
        return switch (type) {
            ...
            case SpellUtils.SpellActionNames.PROJECTILE -> createProjectileAction(json);
    ...
    private static GenericAction createAOEAction(JsonObject json) {
        ...
        if (json.has(SpellUtils.SpellActionProperties.HIT_ACTION)) {
            JsonObject hitActionObject = json.get(SpellUtils.SpellActionProperties.HIT_ACTION).getAsJsonObject();
            String type = hitActionObject.get(SpellUtils.SpellActionProperties.TYPE).getAsString();
    ...
    private static GenericAction createProjectileAction(JsonObject json) {
        float speed = json.get(SpellUtils.SpellActionProperties.SPEED).getAsFloat();
        float lifetime = json.get(SpellUtils.SpellActionProperties.LIFETIME).getAsFloat();
        float width = json.get(SpellUtils.SpellActionProperties.WIDTH).getAsFloat();
        float height = json.get(SpellUtils.SpellActionProperties.HEIGHT).getAsFloat();
        float effectDropoff = json.get(SpellUtils.SpellActionProperties.EFFECT_DROPOFF).getAsFloat();
        boolean hasGravity = json.get(SpellUtils.SpellActionProperties.HAS_GRAVITY).getAsBoolean();
        boolean isBlocking = json.get(SpellUtils.SpellActionProperties.IS_BLOCKING).getAsBoolean();
