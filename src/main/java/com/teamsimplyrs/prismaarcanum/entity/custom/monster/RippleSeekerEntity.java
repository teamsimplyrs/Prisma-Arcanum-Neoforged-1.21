...
public class RippleSeekerEntity extends Monster implements RangedAttackMob {
    ...
    protected void addBehaviorGoals() {
        ...
        this.goalSelector.addGoal(4, new MoveTowardsTargetGoal(this, 0.75d, 12));
        ...
        if (!level().isClientSide) {
            if (attack1Scheduled && target != null && target.isAlive() && !target.isInvisible()) {
                if (attack1Delay > 0) {
                    ...
                    projectile.setProjectileParameters(ATTACK_1_DAMAGE, ATTACK_1_LIFETIME, 0.3f, 1, true, ProjectileMotionType.DEFAULT);

                    projectile.setPos(this.position().add(0f, 0.5f, 0f));
                    projectile.launch(target.position().subtract(this.getForward()).normalize());
